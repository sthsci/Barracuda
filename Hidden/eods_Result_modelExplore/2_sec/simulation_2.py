...
def run_simulation():
	n_killers = 1000
	max_event = 15
	outdir = Path(__file__).resolve().parent / "plots_2"
	...
	# ---- Scenarios (vary p0 and capacity std sigma) ----
	# Normal capacities are rounded to int internally; min_K=0 avoids negatives.
	K0_mean = 2
	sigma_values = [0.0, 0.5, 1.0, 2.0]

	p0_settings = [
		{"name": "p0=1", "p_mode": "deterministic", "p_stochastic_mode": "constant", "p0": 1.0},
		# {"name": "p0=0.7", "p_mode": "stochastic", "p_stochastic_mode": "constant", "p0": 0.7},
		{"name": "p0=0.5", "p_mode": "stochastic", "p_stochastic_mode": "constant", "p0": 0.5},
		# {"name": "p0=0.3", "p_mode": "stochastic", "p_stochastic_mode": "constant", "p0": 0.3},
		{"name": "p0=0.1", "p_mode": "stochastic", "p_stochastic_mode": "constant", "p0": 0.1},
	]

	scenarios = []
	for pset in p0_settings:
		for sigma in sigma_values:
			scenarios.append(
				{
					"name": f"{pset['name']} | sigma={sigma}",
					"p_mode": pset["p_mode"],
					"p_stochastic_mode": pset["p_stochastic_mode"],
					"p0": float(pset["p0"]),
					"alpha": 0,
					"beta": 0,
					"capacity_mode": "normal",
					"capacity_kwargs": {"K0": int(K0_mean), "sigma": float(sigma), "min_K": 0},
				}
			)
