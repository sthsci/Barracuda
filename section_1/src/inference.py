...
def _gamma_marginal_logI(n_t, T: float, mu_lam, sig_lam):
    shape, rate = _gamma_shape_rate_from_mean_sd(mu_lam, sig_lam)
    return (
        pt.gammaln(shape + n_t)
        - pt.gammaln(shape)
        + shape * pt.log(rate)
        - (shape + n_t) * pt.log(rate + float(T))
    )
    ...
    *,
    ...
    with pm.Model() as model:
        ...
        pm.Potential(
            ...
            pt.sum(_poisson_count_constant(n_t, T) + _gamma_marginal_logI(n_t, T, mu_lam, sig_lam)),
        )
    ...
    *,
    ...
    with pm.Model() as model:
        ...
        log_active = (
            ...
            + _gamma_marginal_logI(n_t, T, mu_lam, sig_lam)
        )
        log_zero = pt.logaddexp(
            pt.log(p_zero),
            pt.log1p(-p_zero) + _gamma_marginal_logI(pt.zeros_like(n_t), T, mu_lam, sig_lam),
        )
